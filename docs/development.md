...
均全绿。`v0.1.6` Linux amd64 本地产物预演也已通过版本/commit、静态 ELF、
SHA256 和许可材料自检；这些仍是**dev/本地候选证据**，不是已经发布的新二进制。
由于 `v0.1.5` 自身不能追溯修复，`v0.1.6` 是一次性 bridge release，仍需显式版本
workaround；从 `v0.1.6` 升级到再下一版时，才能首次证明推荐命令端到端闭环。
候选与历史证据见
