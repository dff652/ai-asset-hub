...
18. **TUI Phase E：产品体验与导航 V2**。🚧 **E1/E2/E3.1 已实现，随
    `v0.1.5` 首次发布，并用 `v0.1.6` 正式包复验（2026-07-30）；E3.2 已合入
    `dev`，E3.3 已进入当前开发候选验证，E3.4+ 待实现**：
    ...
    - 🚧 **E3.3 候选（2026-07-30）**：迁移页按 `e` 选择资产组合，零写入显示
      ...
      不发布、不取回、不自动 apply。逐历史发布包绑定与双设备验收留给 E3.4。
...
- `channel.json` 无 JSON schema，但 Go 侧用 `DisallowUnknownFields` +
  `schemaVersion`/`kind` 双检，实际约束比一份 schema 文档更严。
...
门禁均已进入 `v0.1.6` 并完成正式包验收。E3.2 已合入 `dev`，E3.3 换机前置
检查已进入开发候选，二者尚未作为 Release 能力验收；当前后续不足是 E3.4
双设备/失败恢复验收，以及 AI 接口还没有覆盖统一资产状态。
...
| 当前 E3.3 开发分支 | `e 换机检查`、device-private/secret/target/adapter 完整只读报告与可导航 TUI | Core/TUI、完整门禁、两项变异验证、隔离 TTY 与 PR #25 当前 head CI 18/18 已完成；待人工合并 |
...
| N4 | P1 | 🚧 **E3.3 换机前置检查** | 已实现 device-private 排除项、当前设备 secret 可用性、目标支持和 adapter dropped/degraded；零写入门禁、变异验证、TTY 与 PR #25 当前 head CI 18/18 已通过。待人工合并 |
...
检查已进入本地候选验证。E3.4 双设备/失败恢复验收和 E4 设置/i18n 尚未实现。
完整发布收口清单见
