...
| N7 | P2 | 🚧 **E4 设置与 i18n** | N7.0 已确认 auto 语言默认、首选资产库只预填且不保存最近历史，并修订 ADR-0006；N7.1 已完成 typed zh-CN/en 基础与首页 40 个消息/双语 golden，尚余 15 个 production TUI 文件待迁移；仍保持中文默认且不创建偏好文件 |
...
两次合并后主线 CI 均 9/9 全绿。E4 设置/i18n 已完成 N7.0 决策收口和 N7.1
首页双语目录切片；其余页面目录、设置页和偏好文件尚未实现。
完整发布收口清单见
...
N7.1 typed 双语首页目录与 golden ✅ → 其余页面目录实施中。
