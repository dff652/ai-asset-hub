...
| 当前 E3.2 开发分支 | typed publish、显式 versions/pull、连续 diff/apply 与 pull 输出防覆盖 | 本地完整门禁、变异验证、TTY apply/doctor/rollback 闭环和严格 review 已通过；尚待 PR/CI |
...
| N3 | P1 | 🚧 **E3.2 跨设备连续向导** | 当前开发候选已编排 build/typed publish/versions/explicit pull/Phase C；通道和输出目录由用户明确选择，pull 后仍必须 diff + typed apply；本地完整门禁、变异验证、TTY 闭环和严格 review 已通过，待 PR/CI |
...
dogfood 与严格 review 已完成，PR/CI 收口后再进入 E3.3 换机前置检查。E4
...
线上产物、显式升级、legacy no-op 与推荐升级命令修复 ✅ → E3.2 本地候选验证 ✅、
PR/CI 待收口。
