...
| N8 | P2 | 🟡 **规模化资产管理增强评估** | N8.0 已用 22 个真实候选/7 个样本库资产完成边界评估；N8.1 已通过 [PR #34](https://github.com/dff652/ai-asset-hub/pull/34) 合入 `dev`，补齐统一筛选和 manifest v1 来源读取，本地门禁、严格 diff review、真 TTY 状态筛选与合并后 CI 均通过，尚未发布；备份就绪/恢复报告及标签/许可证继续按真实需求触发，见[方案](designs/scalable-asset-management.md) |
...
边界评估 ✅ → N8.1 统一筛选与来源读取（`dev` 已合入，尚未发布）。
