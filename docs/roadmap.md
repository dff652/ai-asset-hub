...
| N7 | P2 | 🚧 **E4 设置与 i18n** | N7.0 已确认 auto 语言默认、首选资产库只预填且不保存最近历史，并修订 ADR-0006；N7.1 已完成 428 个 typed 消息及首页、inventory/资产库管理、diff/apply/二次确认、doctor/rollback、migration 全流程切片，尚余 version 共 2 个 production TUI 文件、29 行含中文文本；仍保持中文默认且不创建偏好文件 |
...
N7.1 首页、inventory/资产库管理、diff/apply、doctor/rollback 和 migration
双语目录切片；version 页面目录、设置页和偏好文件尚未实现。
完整发布收口清单见
...
migration 全流程目录及 golden ✅ → version 页面实施中。
