...
N10.1–N10.3 已发布；N10.4 明确延期。当前优先级是 dogfood 与定位验证，不是扩
功能。搜索/标签/描述、来源与许可证追踪仍由真实需求触发。aiah 仍不实现网络传输、
后台双向同步或云端账户体系。
...
| 顺序 | 动作 | 性质 | 为什么在这个位置 |
|---|---|---|---|
| P1 | ✅ **macOS 支持（开发完成；待带产物正式发布）** | 工程 | 真机探针零生产改动；多平台发布/安装器/形状校验已合入。**线上尚无 darwin 产物**直至 `v0.1.11` Release 验收；分支保护须把 `macos` 设为必需检查 |
...
6. ⬜ **发 `v0.1.11` 带 macOS 产物**，线上验收后按 staged-pin 规则再移动默认 pin（候选检查点见 reviews）。
...
| P | P0 | ✅ **产品定位落地（文档）** | pitch 改为「安全地改 AI 配置 / 多工具语义」；`scan` 为零承诺入口；路线图与研究结论对齐。见[产品定位](research/product-positioning-and-direction.md) |
| D | P0 | ⬜ **真机 dogfood 与反馈** | 用 v0.1.10 跑只读盘点 → 安全应用 → 可选换机对账；记录痛点后再开功能，不默认 N10.4 |
| M | P1 | ⬜ **macOS 原生验收（若要外部用户）** | 交叉编译已有；缺 apply/rollback/hooks 等原生语义验收。自用 Linux 可继续延期 |
| N10.4 | P2 | ⏸️ **隔离演练编排与证据记录** | **明确延期**；仅当手写 evidence 不够用时再开 |
| N8.3/N8.4 | P2 | ⏸️ **规模化描述/标签/索引** | 继续按真实规模需求触发 |

N10.1–N10.3 已随 `v0.1.10` 正式发布。**当前默认下一动作是真实使用与反馈**，不是
新 milestone。不默认做：N10.4、后台同步、云/NAS/Git 客户端、MCP 写操作、继续
堆 TUI。N8.3/N8.4 与新 target 仅按真实需求触发。

产品取舍（路线 C 团队规范可落地、pitch、scan 入口、macOS 门槛）见
