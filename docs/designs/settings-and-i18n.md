...
- 状态：N7.0–N7.2 已实现；N7.1 的 457 条完整双语消息目录和核心页面 golden、
  N7.2 的严格偏好 Core 已于 2026-07-31 完成；设置页和用户可操作的语言切换尚未实现
- 日期：2026-07-30
- 目标阶段：TUI 产品体验 V2 E4
...
当前代码**没有**设置页或用户可操作的语言切换。TUI 仍以中文为兼容默认，但所有
production TUI 用户文案已进入 typed 双语目录，并有 zh-CN/en 核心页面 golden、
help 断言和 catalog 完整性门禁。`internal/preferences` 已能严格读取、校验和
原子保存偏好，但尚未接入 TUI；普通启动不会因此读取或创建偏好文件。设置入口和
发布包验收完成前，不能把“双语设置”写成已支持。
