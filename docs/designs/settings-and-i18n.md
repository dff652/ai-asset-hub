...
- 状态：N7.0–N7.3 已实现；完整双语消息目录、严格偏好 Core、设置页和语言切换已于
  2026-07-31 完成；显示密度、首选资产库预填和正式安装包验收尚未实现
- 日期：2026-07-30
- 目标阶段：TUI 产品体验 V2 E4
...
当前 `dev` 候选已提供“偏好设置 / Preferences”入口、`auto` / `zh-CN` / `en`
选择和 `aiah ui --language` 临时覆盖。无偏好文件时实际按 locale 选择中文或
English；读取始终只读，只有用户在设置页明确选择“保存偏好”才创建或替换文件。

显示密度和首选资产库仍未开放编辑或预填，public `v0.1.6` 也不包含 N7 改动。
README 与上手指南须等 N7.5 正式安装包验收后，才能把“双语设置”标成已发布。
...
N7.3 当前只开放语言选择；设置页中的“重置全部本机偏好”会把语言、显示密度和首选
资产库恢复为安全默认值，但重置本身仍不写文件，必须再次明确选择“保存偏好”。
显示密度选择和首选资产库输入框到 N7.4 才开放。
