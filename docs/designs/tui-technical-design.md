...
2026-07-30 的 [N7 Proposed 方案](settings-and-i18n.md)进一步区分了“业务配置”和
“设备本地 UI 偏好”：后者最多保存语言、首选资产库预填和显示密度，不改变 manifest、
Core 规则或写入授权。该例外尚未实现；实施前必须先修订 ADR-0006 并通过偏好文件
安全门禁。
...
- 不做业务设置/控制面板；E4 只按 N7 Proposed 方案评估三项非业务 UI 偏好；
...
### Phase D2：Doctor 与当前部署回滚

**实现状态：已完成（2026-07-28）。** 普通 `aiah ui` 可从 inventory 或 deployment
页按 `h` 进入 Doctor，直接渲染 `apply.Doctor` 的 deployment、drift 与 findings。
只有报告通过且当前 deployment 带 `backupId`，才开放 `x`；完整输入 `rollback`
后调用 `apply.Rollback`，成功再并行刷新 Doctor 与 inventory。
